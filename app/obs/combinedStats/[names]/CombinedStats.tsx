...
export interface CombinedSessionSettings {
    hours: number,
    hoursBetween: number,
    showAvg: boolean,
    dynamic: boolean,
...
function StatLine({data, settings, split}: { data: any, settings: CombinedSessionSettings, split: string }) {
    const style = {
...
export default function CombinedStats({names, settings, setWidth, setHeight}: {
    names: string,
    settings: CombinedSessionSettings,
    setWidth?: any,
    setHeight?: any
}) {
    const { data } = useSWR(`/stats/api/getCombinedStats/?names=${names}&hours=${settings.hours}&hoursBetween=${settings.hoursBetween}&start=${settings.start}`,
